...
export class ChartsService {
  ...
  parseChartData1(): ChartParsedItem[] {
    return Array.from(
      (Array.from(JSON.parse(JSON.stringify(data))) as  ChartDataItem[])
        .reduce((acc, cur) => {
          if (acc.get(cur.src_office_id)) {
            ...
            acc
              .get(cur.src_office_id)
              .data.qty_delivered.push(cur.qty_delivered);
  ...
  parseGeneralChartData(): ChartParsedItem {
    ...
    const chart: ChartParsedItem = {
      src_office_id: 'general',
      office_name: 'General Chart',
      labels: <string[]>[],
      data: {
        qty_orders: <number[]>[],
        qty_new: <number[]>[],
